...
class BertForSequenceRegression(BertPreTrainedModel):
    ...
    Params:
        `config`: a BertConfig class instance with the configuration to build a new model.
        `num_preds`: the number of values to predict.
    ...
    model = BertForSequenceClassification(config, num_labels)
    preds = model(input_ids, token_type_ids, input_mask)
    ```
    """
    def __init__(self, config, num_preds):
        super(BertForSequenceRegression, self).__init__(config)
        self.num_labels = num_preds
        self.bert = BertModel(config)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
        self.regressor = nn.Linear(config.hidden_size, num_preds)
        self.apply(self.init_bert_weights)
